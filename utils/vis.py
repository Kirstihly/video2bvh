...
def vis_2d_keypoints(
    keypoints, img, skeleton, kp_thresh, alpha=0.7, output_file=None, show_name=False
):
    ...
    while stack:
        ...
        if kp_thresh is None or p_score > kp_thresh:
            ...
            if show_name:
                cv2.putText(
                    mask, parent, p_pos, cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0)
                )
        for child in skeleton.children[parent]:
            if (
                child not in skeleton.keypoint2index
                or skeleton.keypoint2index[child] < 0
            ):
                continue
            stack.append(child)
            ...
            if kp_thresh is None or (p_score > kp_thresh and c_score > kp_thresh):
                cv2.line(
                    mask,
                    p_pos,
                    c_pos,
...
def vis_3d_keypoints_sequence(
    keypoints_sequence, skeleton, azimuth, fps=30, elev=15, output_file=None
):
    ...
    if output_file:
        ...
        if output_file.suffix == ".mp4":
            ...
            raise ValueError(
                f"Unsupported output format." f"Only mp4 and gif are supported."
            )
